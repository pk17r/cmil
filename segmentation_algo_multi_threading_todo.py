...
def segmentation_algo(file_index):
    global show_images, save_intermediate_images, input_dir, output_dir, save_epithelia_and_stroma, output_visualization_dir, save_bins_representation, files #, no_of_threads_to_use
    ...
    if save_bins_representation or show_images:
        ...
        plt.tight_layout()
        if save_bins_representation:
            filename = os.path.join(output_visualization_dir, image_name + " YCbCrCg Channels.png")
            plt.savefig(filename)
            print(filename + " saved")
    ...
    #
    #img_lumma_plus_Cg = img_ycrcb[:,:,0] + img_Cg
    #imshow(img_lumma_plus_Cg, "img_lumma_plus_Cg")
    ...
    rescale_size = 0.2
    combined_image = np.hstack((rescale(img_rgb, rescale_size, anti_aliasing=True), rescale(stroma_img, rescale_size, anti_aliasing=True), rescale(epithelia_img, rescale_size, anti_aliasing=True)))
...
#
#    # run prog here
#    segmentation_algo(thread_num, image_name)
#
